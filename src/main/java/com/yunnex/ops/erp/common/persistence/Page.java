...
public class Page<T> {
    ...
    private Integer startIndex = 0;// 查询开始下标
    ...
    /**
     * 是否使用PaginationInterceptor拦截器的自动计算总数功能。 如果不使用，则需要自己查询总数。一般用于select语句中有子查询的分页语句。
     */
    private boolean isAutoCount = true;
    /**
     * 是否是分页查询，为了标识计算总数时不查询列&不排序
     */
    private boolean isPageQuery = true;

    public static final int NO_PAGE = -2;
    public static final int DEFAULT_LAST = 20;
    ...
    public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize) {
        ...
        // 设置页面分页函数
        String funcNameParam = request.getParameter("funcName");
        if (StringUtils.isNotBlank(funcNameParam)) {
            CookieUtils.setCookie(response, "funcName", funcNameParam);
            this.setFuncName(funcNameParam);
        } else if (request.getParameter("repage") != null) {
            funcNameParam = CookieUtils.getCookie(request, "funcName");
            if (StringUtils.isNotBlank(funcNameParam)) {
                this.setFuncName(funcNameParam);
            }
        }
    ...
    public void initialize() {
        ...
        this.last = (int) (count / (this.pageSize < 1 ? DEFAULT_LAST : this.pageSize) + first - 1);
