...
public class SQLServer2005Dialect implements Dialect {
	...
	public static final int NUMBER_SELECTED = 6;
	public static final int NUMBER_SELECTED_DISTINCT = 15;
     ...
     */
    private static String getLimitString(String querySqlString, int offset, int limit, String limitPlaceholder) {
        StringBuilder pagingBuilder = new StringBuilder();
        String orderby = getOrderByPart(querySqlString);
        String distinctStr = "";

        String loweredString = querySqlString.toLowerCase(Locale.US);
        String sqlPartString = querySqlString;
        if (loweredString.trim().startsWith("select")) {
            int index = NUMBER_SELECTED;
            if (loweredString.startsWith("select distinct")) {
                distinctStr = "DISTINCT ";
                index = NUMBER_SELECTED_DISTINCT;
            }
