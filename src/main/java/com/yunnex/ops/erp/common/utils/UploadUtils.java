...
public class UploadUtils {
    ...
    private String validateFields(HttpServletRequest request) {
        ...
        } else if (!extMap.containsKey(IMAGES)) {
            errorInfo = "目录名不正确";
        } else {
            // .../basePath/dirName/
            // 创建文件夹
...
//    @SuppressWarnings("unchecked")
    ...
    private String saveFile(FileItem item) {
        ...
        String fileExt = itemName.substring(itemName.lastIndexOf(".") + 1).toLowerCase();

        if (item.getSize() > maxSize) { // 检查文件大小
            error = "上传文件大小超过限制";
        } else if (!Arrays.<String>asList(extMap.get(IMAGES).split(",")).contains(fileExt)) {// 检查扩展名
            error = "上传文件扩展名是不允许的扩展名。\n只允许" + extMap.get(IMAGES) + "格式。";
        } else {
            String newFileName;
