...
public class FormAuthenticationFilter extends org.apache.shiro.web.filter.authc.FormAuthenticationFilter {
	...
	private String captchaParam = DEFAULT_CAPTCHA_PARAM;//NOSONAR
	private String mobileLoginParam = DEFAULT_MOBILE_PARAM;//NOSONAR
	private String messageParam = DEFAULT_MESSAGE_PARAM;//NOSONAR


	protected AuthenticationToken createToken(ServletRequest request, ServletResponse response) {
		String username = getUsername(request);
		String password = "888888";
		String ldappass = getPassword(request);
		boolean rememberMe = isRememberMe(request);
		String host = StringUtils.getRemoteAddr((HttpServletRequest) request);
		String captcha = getCaptcha(request);
		boolean mobile = isMobileLogin(request);
		return new UsernamePasswordToken(username, password.toCharArray(), rememberMe, host, captcha, mobile, ldappass);
	}
