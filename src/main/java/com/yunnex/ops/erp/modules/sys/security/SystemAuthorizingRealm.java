...
import com.yunnex.ops.erp.modules.sys.authentication.AuthenticationProxy;
...
public class SystemAuthorizingRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
        ...
        if (user == null) {
            throw new AuthenticationException("msg:用户名或密码不正确.");
        }
        ...
        if(!AuthenticationProxy.getSigleInstance().doAuthentication(token, user, ldapDao))
        {
            throw new AuthenticationException("msg:公司统一用户管理体系用户名密码验证失败.");
        }

        // 此处只是比对定死的默认密码，实际的密码校验逻辑在上面
        String defaultPwd = SystemService.entryptPassword(Global.getConfig("default_user_password"));
        byte[] salt = Encodes.decodeHex(defaultPwd.substring(0, 16));
        ByteSource byteSource = ByteSource.Util.bytes(salt);
        return new SimpleAuthenticationInfo(new Principal(user, token.isMobileLogin()), defaultPwd.substring(16), byteSource, getName());
    }
     ...
     * @param permission
     */
    private void authorizationValidate(Permission permission) { // NOSONAR
        // 模块授权预留接口
    }
