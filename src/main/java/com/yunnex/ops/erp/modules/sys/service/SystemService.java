...
import com.yunnex.ops.erp.common.security.shiro.session.SessionDAO;
...
import com.yunnex.ops.erp.modules.sys.dao.MenuDao;
...
public class SystemService extends BaseService implements InitializingBean {
    ...
    @Transactional(readOnly = false)
    public void saveLdapUser(User user) {
        user.preInsert();
        userDao.insert(user);
        if (StringUtils.isNotBlank(user.getId())) {
            // 更新用户与角色关联
    ...
    @Transactional(readOnly = false)
    public void updatePasswordById(String id, String loginName, String newPassword, String updatePasswordFlag) {
        User user = new User(id);
        user.setPassword(entryptPassword(newPassword));
        user.setUpdatePasswordFlag(Integer.parseInt(updatePasswordFlag));
    ...
    public static boolean printKeyLoadMessage() {
        ...
        sb.append("\r\n======================================================================\r\n");
        sb.append("\r\n    欢迎使用 " + Global.getConfig("productName") + "  - Powered By 运营平台部\r\n");
        sb.append("\r\n======================================================================\r\n");
    ...
    public int getUpdatePasswordFlag(String userid) {
        return userDao.getUpdatePasswordFlag(userid);

    }

    public List<String> findAllNormalUserIdList() {
        return userDao.findAllNormalUserIdList();
    }

    public List<User> getUserByRoleName(String roleName) {
        if (StringUtils.isEmpty(UserUtils.getUser().getId())) {
            List<User> list = userDao.getUserByRoleName(roleName);
            filterAdmin(list);
            return list;
        }
        Role roleFriends = getRoleByEnname(roleName);
        List<User> userListFriends = findUser(new User(new Role(roleFriends.getId())));
        return userListFriends;
    }

    private void filterAdmin(List<User> list) {
        if (list.size() > 1) {
