 ...
 * @date 2018年7月4日
 */
public class DeliveryFlowfirstPlanBasicProcessHandler implements ProcessHandler {
    ...
    public void start(List<SplitGoodForm> request, ErpOrderOriginalInfo erpOrderOriginalInfo, Map<String, Object> response,
        ...
        logger.info("首次上门服务（基础版）服务流程 ,[{}]", request);
        ...
        // 订单里的商品包含服务项目“掌贝平台交付服务”和“首次上门服务（基础版）”；`
        if (Optional.ofNullable(zbGoodService).isPresent() && notExpired(zbGoodService.getExpirationTime()) && Optional.ofNullable(kclGoodService)
                        .isPresent() && notExpired(kclGoodService.getExpirationTime())&&!isStartFlow(response)) {
            vars.put(FlowConstant.SERVICETYPE, DeliveryFlowConstant.SERVICE_TYPE_KE_CHANG_LAI_BASIC);
        ...
        logger.info("首次上门服务（基础版）服务流程 结束,[{}]", vars);
