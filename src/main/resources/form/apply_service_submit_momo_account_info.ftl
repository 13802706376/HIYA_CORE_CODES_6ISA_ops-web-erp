...
function submit_${taskId!''}submitForm1060(isLocation){
	var channels =[]; 
	var channelName =[];
	...
	var submit = function (v, h, f) {
			...
			$.post("../jyk/flow/apply_service_submit_momo_account_info", {
				channelList : channels,
				isFinished:isFinished,
				taskId:$("#${taskId!''}taskId1060").val(),
				procInsId:$("#${taskId!''}procInsId1060").val()
			}, 
			function(data) {
				if (data.result) {
					if(isLocation==1){
	...
	$.jBox.confirm("确定陌陌推广开户吗？", "提示", submit);
}
</script>
<input type='hidden' id='${taskId!''}taskId1060' name='${taskId!''}taskId1060' value="${taskId!''}"/>
<input type='hidden' id='${taskId!''}procInsId1060' name='${taskId!''}procInsId1060'  value="${procInsId!''}"/>
<div class="act-rs-item  ${detailType!''}  ${followTaskDetailWrap!''}">
	<div class="act-rs-left floatleft">
		<div class="act-rs-title padding15">
			...
			<div class="listSubmit">
				<button  onclick="submit_${taskId!''}submitForm1060()" type="button" class="makesrue btn">确定完成</button>
		...
		<div class="act-rs-form padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					...
					>
					<span class="subTask" value="1">通过邮件提交陌陌推广素材给业管（包括落地页链接、ICP备案号等）</span>
				</label>
			</div>
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<input type="checkbox" value="2" name="${taskId!''}channelname1060"
						<#if subTaskList?? && (subTaskList?size > 0) ><#list subTaskList as subTask>
					 	<#if subTask.subTaskId == "2" && subTask.state=="2">
						 		 checked="true"
						 	</#if>
						 </#list>
					  </#if>
					>
					<span class="subTask" value="2">确定陌陌推广开户完成</span>
				</label>
			</div>
		</div>
	</div>
	...
	<div class="act-process-left-footer" >
		...
		<div class="footer-btn floatright">
			<button style="button" class="btn btn-large"  onclick="submit_${taskId!''}submitForm1060(1)">确定完成</button>
