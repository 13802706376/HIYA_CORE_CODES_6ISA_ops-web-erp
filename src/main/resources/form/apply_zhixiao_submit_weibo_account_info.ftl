...
function submit_${taskId!''}submitForm1014(isLocation){
	var channels =[]; 
	var channelName =[];
	$('input[name="${taskId!''}channelname1014"]:checked').each(function(){ 
		channels+=$(this).val()+","; 
		channelName.push($(this).next('span').text());
	});
	if(channelName.length === 0) {
		return;
		$.jBox.tip("选勾选确定微博推广开户成功！", 'error');
	}
	var submit = function (v, h, f) {
			...
			$.post("../jyk/flow/apply_zhixiao_submit_weibo_account_info", {
	...
	$.jBox.confirm("确定微博推广开户完成吗？", "提示", submit);
}
</script>
...
<div class="act-rs-item  ${detailType!''}  ${followTaskDetailWrap!''}">
	<div class="act-rs-left floatleft">
		<div class="act-rs-title padding15">
			...
			<div class="listSubmit">
				<button  onclick="submit_${taskId!''}submitForm1014()" type="button" class="makesrue btn">确定完成</button>
		...
		<div class="act-rs-form padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<input type="checkbox"  value="1" name="${taskId!''}channelname1014">
					<span class="subTask" value="1">确定微博推广开户成功</span>
				</label>
			</div>
		</div>
	</div>
	...
	<div class="act-process-left-footer" >
		...
		<div class="footer-btn floatright">
			<button style="button" class="btn btn-large" onclick="submit_${taskId!''}submitForm1014(1)">确定完成</button>
