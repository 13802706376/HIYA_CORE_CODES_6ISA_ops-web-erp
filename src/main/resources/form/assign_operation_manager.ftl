...
function submit_${taskId!''}submitForm1002(isLocation){
	var channels =[]; 
	var channelName =[];
	if($("#${taskId!''}operationManager option:selected").val() === ""){
		return;
		$.jBox.info("请选择运营经理");
	}
	var submit = function (v, h, f) {
			...
			$.post("../jyk/flow/assign_operation_manager", {
					operationManager : $("#${taskId!''}operationManager option:selected").val(),
	...
	$.jBox.confirm("确认指派运营经理为["+$("#${taskId!''}operationManager option:selected").text()+"]吗？", "提示", submit);
}
</script>
<input type='hidden' id='${taskId!''}taskId1002' name='${taskId!''}taskId' value="${taskId!''}"/>
<input type='hidden' id='${taskId!''}procInsId1002' name='${taskId!''}procInsId'  value="${procInsId!''}"/>
		...
		<div class="act-rs-form form-horizontal padding15">
			<div class="rs-label">
				<div class="control-group">
					<label class="subTask" value="1">指派运营经理：
					<#if taskUserList?? && (taskUserList?size > 0) >
					 	 <select id="${taskId!''}operationManager" class="input-medium">
