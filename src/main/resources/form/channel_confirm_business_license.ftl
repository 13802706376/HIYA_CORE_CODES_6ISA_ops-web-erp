...
function submit_${taskId!''}submitForm1008(isLocation){
	...
	if($.trim($('input[name="${taskId!''}nextLicenseTime"]').val()) === '' && $('input[name="${taskId!''}channelname1008"]:checked').val() === '2') {
	...
	var submit = function (v, h, f) {
			...
			$.post("../jyk/flow/channel_confirm_business_license", {
...
$(function() {
	 ...
	 <#if promotionTime??> <#else> <#if nextLicenseTime??> 
	...
	<div class="act-rs-left floatleft">
		...
		<#if vars?exists>
			<#list vars?keys as key> 
				<#if vars["nextLicenseTime"]??>
					<#assign nextLicenseTime = vars["nextLicenseTime"]>
				</#if>
			</#list>
		</#if>
		<div class="act-rs-form form-horizontal padding15">
			...
			<div class="rs-label">
				<div class="stdiv">
					<label style="margin-right:50px;">
						...
						</#if> type="radio" value="1" name="${taskId!''}channelname1008" class="${taskId!''}selectpromotionTime" onclick="yes_${taskId!''}()">有营业执照
					</label>
					<label>
						<input type="radio" <#if nextLicenseTime??> checked="checked"  </#if>  value="2" name="${taskId!''}channelname1008" class="${taskId!''}selectpromotionTime" onclick="not_${taskId!''}()">没有营业执照
					</label>
				</div>
			</div>
			...
			<div class="rs-label hide" id="${taskId!''}nosurepromotionTime">
				<div class="control-group">
					...
					<div class="controls">
						<input name="${taskId!''}nextLicenseTime" id="next" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate" value="${nextLicenseTime!""}" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',isShowClear:false,minDate:'%y-%M-%d'});">
					</div>
				</div>
			</div>
