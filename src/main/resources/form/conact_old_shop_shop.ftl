<script type="text/javascript">
	function submit_${taskId!''}submitForm1055(isLocation){
		...
		if(channelName.length === 3 && hasZzqq!= ''  && typeof hasZzqq !== 'undefined' ) {
			isFinished = true;
		}
		...
		var submit = function (v, h, f) {
				...
				$.post("../sdi/flow/conact_old_shop_shop", {
		...
		$.jBox.confirm("确认完成联系老商户吗？", "提示", submit);
	}
</script>
		...
		<div class="act-rs-form padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					...
					<span class="subTask" value="1">电话联系商户，介绍新增服务内容</span>
				</label>
			</div>
			
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<span class="subTask" value="2">确认商户是否资质齐全：</span>
			...
			<div class="rs-label">
				<label class="rs-label-wrapper">
					...
					<span class="subTask" value="3">服务卡发放：微信群发放服务卡</span>
				</label>
			</div>
			...
			<div class="rs-label">
				<label class="rs-label-wrapper">
					...
					<span class="subTask" value="4">资料补充：微信群发掌贝智慧服务中心（资料收集小程序）链接，提醒商户使用掌贝商户后台登录帐号密码进入提交所需补充的资料</span>
				</label>
			</div>
		</div>
	</div>
