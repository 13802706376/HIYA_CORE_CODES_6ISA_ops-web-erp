...
function submit_${taskId!''}submitForm1055(isLocation){
	...
	$.jBox.confirm("确认联系服务商，获取服务商微信号和商户联系方式吗？", "提示", submit);
}
		...
		<div class="act-rs-form padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<input type="checkbox" <#if (subTaskList[0])?? && subTaskList[0].state == "1">checked="checked"</#if>  valure="1" name="${taskId!''}channelname1055">
					<span class="subTask" value="1">联系服务商，获取服务商微信号和商户联系方式</span>
				</label>
			</div>
		</div>
	</div>
