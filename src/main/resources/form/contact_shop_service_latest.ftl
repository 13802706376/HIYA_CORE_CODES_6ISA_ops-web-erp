<script type="text/javascript">
	...
	function submit_${taskId!''}submitForm12(isLocation){
		...
		var submit = function (v, h, f) {
				...
				$.post("../jyk/flow/accountService/contact_shop_service_latest", {
						storeId : storeId,
						storeName : storeName,
...
<div class="act-rs-item  ${detailType!''}  ${followTaskDetailWrap!''}">
	<div class="act-rs-left floatleft">
		...
		<div class="act-rs-form padding15">
			...
			<div class="rs-label hide" id="${taskId!''}pipe">
				<span class="rs-label-wrapper" style="display: inline-block;">
					<span class="subTask" value="2">推广通道：</span>
					<input type="checkbox" name="${taskId!''}channels" value="1" class="${taskId!''}channels" <#if (vars.checkFriendFlag)?? && (vars.checkFriendFlag=="Y")> checked="checked" </#if>/>微信朋友圈
					<input type="checkbox" name="${taskId!''}channels" value="2" class="${taskId!''}channels" <#if (vars.checkMicroblogFlag)?? && (vars.checkMicroblogFlag=="Y")> checked="checked" </#if>/>新浪微博
					<input type="checkbox" name="${taskId!''}channels" value="3" class="${taskId!''}channels" <#if (vars.checkMomoFlag??) && (vars.checkMomoFlag=="Y")> checked="checked" </#if>/>陌陌
				</span>
			</div>
