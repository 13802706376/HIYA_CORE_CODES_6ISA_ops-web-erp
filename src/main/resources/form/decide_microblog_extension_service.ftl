<script type="text/javascript">
	function submit_${taskId!''}submitForm12(isLocation){
		...
		var submit = function (v, h, f) {
				...
				$.post("../jyk/flow/accountService/decide_microblog_extension_service", {
		...
		$.jBox.confirm("确认完成判断是否进行微博推广开户吗？", "提示", submit);
	}
		...
		<div class="act-rs-form padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<span class="subTask" value="2">是否进行微博推广开户：</span>
					<input type="radio" name="${taskId!''}license" value="1" class="${taskId!''}license"/>是
					<input type="radio" name="${taskId!''}license" value="2" class="${taskId!''}license"/>否
				</label>
			</div>
		</div>
	</div>
