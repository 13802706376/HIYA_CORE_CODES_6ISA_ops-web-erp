<script type="text/javascript">
	function submit_${taskId!''}submitForm1057(isLocation){
		...
		if(isPass === '') {
			$.jBox.tip("请确认是否需要进行微信支付进件！", 'error');
			return;
		}
		var submit = function (v, h, f) {
				...
				$.post("../jyk/flow/accountZhixiao/decide_wechat_in_zhixiao", {
						isPass : isPass,
		...
		$.jBox.confirm("确认是否需要进行微信支付进件吗？", "提示", submit);
	}
...
<input type='hidden' id='${taskId!''}taskId1057' name='taskId1057' value="${taskId!''}"/>
<input type='hidden' id='${taskId!''}procInsId1057' name='procInsId1057'  value="${procInsId!''}"/>
		...
		<div class="act-rs-form padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<span class="subTask" value="1">是否需要进行微信支付进件：</span>
