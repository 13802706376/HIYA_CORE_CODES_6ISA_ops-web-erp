...
function submit_${taskId!''}submitForm1055(isLocation){
	...
	if(channelName.length === 0 && channels === '') {
		return;
		$.jBox.tip("请确保收到服务商提交的朋友圈开户资料并存档, 发起朋友圈授权！", 'error');
	}
	var submit = function (v, h, f) {
			...
			$.post("../jyk/flow/accountService/friends_promote_info_review_service", {
			...
			function(data) {
				if (data.result) {
					$.jBox.info("收到服务商提交的朋友圈开户资料并存档, 发起朋友圈授权任务完成.");
					if(isLocation==1){
	...
	$.jBox.confirm("确认完成该任务吗？", "提示", submit);
}
		...
		<div class="act-rs-form padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					...
					<span class="subTask" value="1">确保收到服务商提交的朋友圈开户资料并存档, 发起朋友圈授权</span>
				</label>
			</div>
			...
			<div class="rs-label">
				<label>
					<span>选择的门店：${storeName!''}&nbsp;&nbsp;&nbsp;
						<a href="javascript:;" class="openFriendsForm" data-datas="${storeId!''},service,edit">存档</a>
					</span>
