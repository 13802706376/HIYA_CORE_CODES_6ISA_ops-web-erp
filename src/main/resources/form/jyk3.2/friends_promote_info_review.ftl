<script type="text/javascript">
	function submit_${taskId!''}submitForm12(isLocation){
		...
		var submit = function (v, h, f) {
				...
				$.post("../friends/flow/3p2/friends_promote_info_review", {
		...
		$.jBox.confirm("确认朋友圈推广开户资料复审吗？", "提示", submit);
	}
		...
		<div class="act-rs-form padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					...
					<input type="radio" name="${taskId!''}isPass" value="Y" class="${taskId!''}isPass"/>是
					<input type="radio" name="${taskId!''}isPass" value="N" class="${taskId!''}isPass"/>否
				</label>
			</div>
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<span>选择的门店：${(storeName)!''}&nbsp;&nbsp;&nbsp;<a href="javascript:;" class="openFriendsForm" data-datas="${storeId!''},zhixiao">查看</a>&nbsp;&nbsp;&nbsp;<a href="../../download/adv/friends?storeId=${storeId!''}" class="downloadForm">《直销商户朋友圈推广开户资料业管审核表》</a></span>
				</label>
			</div>
		</div>
	</div>
