...
<div class="task-detail-wrapper">
   <h3 class="task-title">微博充值资料修改</h3>
   <div class="task-sider">
      <div class="task-sider-title"><span>修改建议</span></div>
      <div class="textarea-wrap">
         <textarea    class="content-textarea"><#if (flowdata.microblogRechargeCheckFailReason)?? >${flowdata.microblogRechargeCheckFailReason}</#if></textarea>
      </div>
   </div>
   <div class="task-sider">
		   ...
		   <#list weiboReachargeLists as weiboReachargeInfo>
	         <div class="border-bottom" name="list-label">
	            <dl class="content-wrap">
	               <dt>微博账号：</dt>
	               <dd  name="${taskId!''}accountNo1014"  class="margin-left-0">${weiboReachargeInfo.weiboAccountNo!''}</dd>
	               <input type="hidden" name="${taskId!''}Weiboid"  value="${weiboReachargeInfo.id!''}">
	            </dl>
	            <dl class="content-wrap">
	               <dt>输入申请充值金额（元）：</dt>
	               <dd>
	                  <input type="text" name="${taskId!''}applyAmount" value="${weiboReachargeInfo.applyAmount?c}" class="content-input">
	               </dd>
	            </dl>
	         </div>
           </#list>      
   ...
   var task_flow_version = '3.2';
	...
	var attr = [];
		$("div[name=list-label]").each(function(obj){
			var	weiboid =$("input[name=${taskId!''}Weiboid]",obj).val(),
	...
	var submit = function (v, h, f) {
			...
			$.post("../jyk/flow/3p2/microblog_recharge_modify_3.2", {
