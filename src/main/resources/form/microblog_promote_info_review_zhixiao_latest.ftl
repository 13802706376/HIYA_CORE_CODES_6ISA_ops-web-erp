<script type="text/javascript">
	function submit_${taskId!''}submitForm12(isLocation){
		var submit = function (v, h, f) {
			if (v == 'ok') {
				...
				$.post("../jyk/flow/accountZhixiao/microblog_promote_info_review_zhixiao_latest", {
						isPass : $('input[name="${taskId!''}isPass"]:checked').val(),//isPass,
						openOrTrans : $('input[name="${taskId!''}openOrTrans"]:checked').val(),//openOrTrans,
	...
	$(function(){
			...
			var time2 = Date.parse(new Date(taskStarterDate));
			var nDays = Math.abs((time2 - time1)/1000/3600/24);
			if(nDays>4)
			{
				$("#colorred").show();
			}
		...
		<div class="act-rs-form padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					...
					<input type="radio"  <#if (openOrTrans)?? && openOrTrans == "O">checked="checked"</#if> name="${taskId!''}openOrTrans" value="O" class="${taskId!''}openOrTrans"/>新开户
					<input type="radio" <#if (openOrTrans)?? && openOrTrans == "T">checked="checked"</#if> name="${taskId!''}openOrTrans" value="T" class="${taskId!''}openOrTrans"/>转户
				</label>
			</div>
			
					<span  id="colorred" style="display:none;color:red;" value="1">该任务持续超过4个工作日,与运营顾问沟通是否重新开户，重新开户的，选择不通过，并备注说明转户异常，进行新开户</span>
					
			
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<span class="subTask" value="1">是否审核通过：</span>
					<input type="radio" name="${taskId!''}isPass" checked="checked" value="1" class="${taskId!''}isPass"/>是
