...
function submit_${taskId!''}submitForm12(isLocation){
	...
	var submit = function (v, h, f) {
			...
			$.post("../jyk/flow/operation_adviser_contact_shop", {
	...
	$.jBox.confirm("确认与商户对接（运营顾问）吗？", "提示", submit);
}
</script>
		...
		<div class="act-rs-form padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					...
					>
					<span class="subTask" value="1">电话联系商户，了解商户资质信息（新商户需获取微信号）</span>
				</label>
			</div>
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<input type="checkbox" value="2" name="${taskId!''}channelname12"
					<#if subTaskList?? && (subTaskList?size > 0) ><#list subTaskList as subTask>
					...
					>
					<span class="subTask" value="2">同步商户资质信息给策划专家</span>
				</label>
			</div>
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<input type="checkbox"  value="3" name="${taskId!''}channelname12" 
					<#if subTaskList?? && (subTaskList?size > 0) ><#list subTaskList as subTask>
					...
					>
					<span class="subTask" value="3">确保商户进入聚引客商户运营服务微信群</span>
				</label>
			</div>
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<input type="checkbox"  value="4" name="${taskId!''}channelname12" 
					<#if subTaskList?? && (subTaskList?size > 0) ><#list subTaskList as subTask>
					 	<#if subTask.subTaskId == "4" && subTask.state=="1">
						 		 checked="true"
						 	</#if>
						 </#list>
					  </#if>
					>
					<span class="subTask" value="4">在商户运营服务微信群介绍各角色，发放发服务卡和商户资料收集表</span>
				</label>
			</div>
		</div>
	</div>
