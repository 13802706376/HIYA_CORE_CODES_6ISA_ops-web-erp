...
function submit_${taskId!''}submitForm1055(isLocation){
	...
	if(channelName.length === 0 && channels === '') {
		return;
		$.jBox.tip("请确保服务商提交微博推广开户资料！", 'error');
	}
	var submit = function (v, h, f) {
			...
			$.post("../jyk/flow/accountService/perfect_microblog_promote_info_service", {
	...
	$.jBox.confirm("确保服务商提交微博推广开户资料吗？", "提示", submit);
}
		...
		<div class="act-rs-form padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					...
					<span class="subTask" value="1">确保服务商提交微博推广开户资料</span>
				</label>
			</div>
		</div>
	</div>
