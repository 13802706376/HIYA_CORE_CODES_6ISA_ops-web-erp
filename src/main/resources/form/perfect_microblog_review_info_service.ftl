<script type="text/javascript">
	function submit_${taskId!''}submitForm1002(isLocation){
		...
		if(channelName.length === 0 && channels === '') {
			$.jBox.tip("请确保微博推广开户资料提审完成并存档！", 'error');
			return;
		}
		var submit = function (v, h, f) {
				...
				$.post("../jyk/flow/accountService/perfect_microblog_review_info_service", {
		...
		$.jBox.confirm("确定微博推广开户资料提审完成并存档吗？", "提示", submit);
	}
</script>
		...
		<div class="act-rs-form form-horizontal padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					...
					<span class="subTask" value="1">微博推广开户资料提审完成并存档</span>
				</label>
			</div>
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<span>选择的门店：${storeName!''}&nbsp;&nbsp;&nbsp;<a href="javascript:;" class="openWeiboForm" data-datas="${storeId!''},service,edit">存档</a></span>
				</label>
			</div>
		</div>
	</div>
