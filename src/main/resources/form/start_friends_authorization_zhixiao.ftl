...
function submit_${taskId!''}submitForm1055(isLocation){
	...
	var submit = function (v, h, f) {
			...
			$.post("../jyk/flow/accountZhixiao/start_friends_authorization_zhixiao", {
	...
	$.jBox.confirm("确认完成发起朋友圈授权吗？", "提示", submit);
}
		...
		<div class="act-rs-form padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					...
					<span class="subTask" value="1">发起以下门店的朋友圈授权</span>
				</label>
			</div>
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<span>已选择的门店：${storeName!''}</span>
				</label>
			</div>
		</div>
	</div>
