<script type="text/javascript">
	function submit_${taskId!''}submitForm1002(isLocation){
		...
		if(channelName.length === 0 && channels === '') {
			return;
			$.jBox.tip("请确认完成确认提交微信支付进件！", 'error');
		}
		...
		var submit = function (v, h, f) {
				...
				$.post("../sdi/flow/wechat_pay_state_shop", {
		...
		$.jBox.confirm("确定完成确认提交微信支付进件吗？", "提示", submit);
	}
</script>
		...
		<div class="act-rs-form form-horizontal padding15">
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<#if storeId??>
						<span class="subTask" value="1">1.对商户提交的<a href="../store/basic/erpStoreInfo/wxpay/?shopId=${shopMainId!''}&storeId=${storeId!''}&isMain=${isMain!''}&from=taskList">微信支付进件资料</a>进行审核，确保资料的完整性和正确性；</span>
					<#else>
						<span class="subTask" value="1">1.对商户提交的<a href="../store/basic/erpStoreInfo/urlErpShopInfoList?id==${shopMainId!''}&from=taskList">微信支付进件资料</a>进行审核，确保资料的完整性和正确性；</span>
					</#if>
				</label>
			</div>
			...
			<div class="rs-label">
				<label class="rs-label-wrapper">
					<span class="subTask" value="3">3. 将商户的资料补全，并完成<a href="../store/basic/erpStoreInfo/payOpen/?id=${shopMainId!''}&from=taskList&type=wechat">微信支付进件</a>。</span>
				</label>
			</div>
		</div>
	</div>
