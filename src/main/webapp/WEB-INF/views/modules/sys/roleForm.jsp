...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			var treeOnCheckHandler = function(cb) {
				var oldName = $('#oldName').val();
				var html = '<div style="padding:10px;">勾选“高级修改”意味着拥有“【'+oldName+'】”这一角色的成员将能修改该成员有权限查看的所有商户的所有门店资料，不管该资料是否已经通过审核。若您确认开放该权限给“【'+oldName+'】”，请在下面的输入框中输入“confirm”。</div><div class="from" style="margin:0 0 10px 10px"><input type="text" id="confirm" name="confirm" /><div id="tips" style="color:#e20; display:none;">输入错误，请重新输入！</div></div>',
					title = '温馨提示',
					buttons = { '取消': 'cancel', '确定': 'ok'},
					submit = function(v, h, f) {
						var confirm = $.trim($(h).find('#confirm').val());
						if (v === 'ok' && confirm !== 'confirm') {
							$(h).find('#tips').show();
							return false;
						};
						cb(v);
			...
			var setting = {
				...
				callback:{
					...
					onCheck: function(ev, treeId, treeNode) {
						var n = 0;
						var flag = treeNode.checked && (treeNode.id === 'dc5c3a4cdee3476d90a6da94e2c74487' || treeNode.name.indexOf('通过审核的资料也能改') !== -1);
						if (flag && n === 0) {
							treeOnCheckHandler(function(v) {
								if (v !== 'ok') {
									n = 0;
									tree.checkNode(treeNode, false, true, true);
								} else {
									n = 1;
								}
							});
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="type" class="input-medium">
					<form:option value="erp">ERP角色</form:option>
					<form:option value="agent">服务商角色</form:option>
				</form:select>
				<span class="help-inline"></span>
			</div>
		</div>
		<div class=" control-group" id="ou">
