...
<head>
	...
	<script type="text/javascript" src="${contextpath}/dwr/interface/SendMsg.js"></script> 

	<script type="text/javascript">
	
	//解决弹出错误框的问题，覆盖 defaultErrorHandler
 	/*
	...
	//DWR唤起触发方法，方法里面实现按具体需要可修改
		function show(title,content,url,userid){  
	    	/*console.log(title);
	    	console.log(content);
	    	console.log(url);*/
	    	var dwruserid=$("#dwruserid").val();
	    	if(userid==dwruserid){
		    	var opsTips = new opsWindowTips({
		            title: title,
		            link: url,
		            contains: content
		        }, function(opt) {
		...
		$(document).ready(function() {
			// 替换dwr 》 engine.js 中_handlePollRetry 方法，解决ReverseAjax.dwr 多个pending，导致请求阻塞问题
			// 添加 pollTimes变量，加入pollTimes<=1 的判断来觉得是否执行 dwr.engine._poll()方法
			// dwr.engine._poll() 中会发起 ReverseAjax.dwr 请求
			...
			//
			//
			$(document).on('click', '.accordion-body a', function() {
				removeSessionStorageItem();
				return true;
			});
			function removeSessionStorageItem() {
				// 用户在各个任务详情页面操作完后，返回任务列表页面记住列表页的筛选状态临时解决办法
				// 进入页面先清除各个任务列表页面的sessionStorage，以免干扰用户后续的操作
				top.globalUtil.sessionStorageRemove('beiyiTaskFlowToWhere');
				top.globalUtil.sessionStorageRemove('taskListFormDataBeiyi');
				top.globalUtil.sessionStorageRemove('taskListFormData');
				top.globalUtil.sessionStorageRemove('teamTaskListFormData');
				top.globalUtil.sessionStorageRemove('beiyi_task_list_page');
				top.globalUtil.sessionStorageRemove('followTaskListData');
			}
		});
	...
	<div id="main">
				...
				<ul id="userControl" class="nav pull-right">
					...
					<li id="userInfo" class="dropdown">
						...
						<ul class="dropdown-menu">
							...
							<c:if test="${fns:getUser().type == 'erp' and fns:getUser().id != '1'}">
								<li><a href="http://cas.corp.yunnex.com/cas/login" target="_blank"><i
										class="icon-lock"></i>&nbsp;
									修改密码</a></li>
							</c:if>
							<c:if test="${fns:getUser().type == 'agent'}">
	    ...
	    <div class="container-fluid">
		    ...
		    <%-- <div id="footer" class="row-fluid">
	            Copyright &copy; 2012-${fns:getConfig('copyrightYear')} ${fns:getConfig('productName')} - Powered By <a href="http://www.yunnex.com" target="_blank">Yunnex</a> ${fns:getConfig('version')}
			</div> --%>
		</div>
	</div>
	<input type="hidden" id="dwruserid" value="${fns:getUser().id}" />
	<script type="text/javascript"> 
		...
		function wSize(){
			var minHeight = 500, minWidth = 1150;
			var strs = getWindowSize().toString().split(",");
			htmlObj.css({"overflow-x":strs[1] < minWidth ? "auto" : "hidden", "overflow-y":strs[0] < minHeight ? "auto" : "hidden"});
			mainObj.css("width",strs[1] < minWidth ? minWidth - 10 : "auto");
			frameObj.height((strs[0] < minHeight ? minHeight : strs[0]) - headerObj.height() - (strs[1] < minWidth ? 24 : 10));
		...
		function wSizeWidth(){
			...
			if (strs) {
				if (strs[1] < 1590) {
					$(window.frames["mainFrame"].document).find("#workflowListActTabBtngroup").show();
				} else {
					$(window.frames["mainFrame"].document).find("#workflowListActTabBtngroup").hide();
				}
			}
