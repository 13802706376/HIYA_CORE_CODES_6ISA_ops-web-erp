...
<body>
	<div class="act-top positionrelative">
		<ul class="nav nav-tabs">
			...
			<li class="acttitle select-team-li">
				<%--<input type="text" id="select-teamUser" class="hide" style="width: 160px;">--%>
	...
	<div class="act-wrap">
		<form:form id="workFlowQuery" modelAttribute="workFlowQueryForm" action="${ctx}/workflow/teamtasklist" method="post" class="form-horizontal">
				...
				<div class="select-wrap clear">
					...
					<div class="select-catgry" style="width:10%;">
						<dl class="sca">
							<dt>订单加急</dt>
							<dd>
								<label><form:checkbox path="hurryFlag" value="1"/>&nbsp;加急</label><br/>
								<label><form:checkbox path="timeoutFlag" value="Y"/>&nbsp;有超时风险</label>
							</dd>
						</dl>
					</div>
					<div class="select-catgry" style="width:10%;">
				...
				<c:if test="${flowFrom.flowMark=='jyk_flow'}">
					<div class="mteamTaskItem act-content-box act-content-flowfrom clear istoggle">
						<div class="act-content-slide act-content-slide-left positionrelative">
							<div class="positionrabsolute workflow-tips-div">
	                            <c:if test="${flowFrom.timeoutFlag =='Y'}">
									<a class="act-tips-timeout showTaskPocess" title="风险" href="javascript:;"
									   data-pid="${flowFrom.act.taskId },${flowFrom.act.procInsId }">
	                                    <span><i class="icon-exclamation-sign"></i></span>
	                                </a>
	                            </c:if>
								<c:if test="${flowFrom.hurryFlag =='1'}">
									<div class="act-tips-word">
										<span>急</span>
									</div>
								</c:if>
							</div>
							...
							<c:forEach items="${flowFrom.subTaskStrList }" var="i">
								<div class="act-rs-left floatleft">
									<div class="act-rs-title padding15">
										<h3><a href="../workflow/taskDetail?detailType=teamtasklist&taskId=${i.taskId}&taskDefKey=${i.subTaskDetail}&startDate=${i.startDate}&endDate=${i.endDate}&splitId=${i.busId}&taskUser=${i.assigneeName}&procInsId=${i.procInsId}&readOnly=true&followTaskDetailWrap=1">${i.taskName}</a></h3>
										<a href="javascript:;"  style="float: right;">${i.assigneeName}</a>
										<a href="javascript:;" class="select-user open-select-user" style="float: left;" onclick="syncSelect('${i.taskId}', '${i.assigneeName}','${i.userId}')"></a>
									</div>
					...
					<div class="mteamTaskItem act-content-box act-content-flowfrom clear istoggle">
						...
						<div
							...
							<c:forEach items="${flowFrom.subTaskStrList }" var="i">
								<div class="act-rs-left floatleft">
									<div class="act-rs-title padding15">
										<h3><a href="../workflow/taskDetail?detailType=teamtasklist&taskId=${i.taskId}&readOnly=true">${i.taskName}</a></h3>
										<a href="javascript:;"  style="float: right;">${i.assigneeName}</a>
										<a href="javascript:;" class="select-user open-select-user" style="float: left;" onclick="syncSelect('${i.taskId}', '${i.assigneeName}','${i.userId}')"></a>
									</div>
								</div>
								<div class="act-rs-right">
									<div class="padding15"  style="height: 60px;">
										...
										<div class="act-jd-word positionrelative"><div class="acjw positionrabsolute" style="left: 80%;">${i.subTaskConsumTime}%</div>
	...
	<script type="text/javascript">
		...
		function submit(flag) {
			sessionStorage.removeItem('teamTaskListFormData');
			var taskState=""; 
			var goodsTypeList ="";
		...
		function setTaskListFormData(arr) {
			var formDta = {
				pageNo: $('#pageNo').val(),
				...
				_timeoutFlag: $('#timeoutFlag1').attr('checked') ? 'on' : '',
				timeoutFlag: $('#timeoutFlag1').attr('checked') ? 'Y' : '',
				//shopName: $.trim($('#shopName').val()),
				teamUserId: $('#teamUserId').val(),
				teamId: $('#teamId').val()
			}
			sessionStorage.setItem('teamTaskListFormData', JSON.stringify(formDta));
		}
			...
			var act = {
		                    ...
		                    if (result && Array.isArray(result)) {
		                    	for (var i = 0, l = result.length; i < l; i++) {
		                    		if (result[i].teamId === tid) {
		                    			...
		                    			var taskListFormData = sessionStorage.getItem('teamTaskListFormData') || '{}';
				...
				setFormData: function() {
					...
					if (taskListFormData) {
						...
						$('#teamId').val(o.teamId);
						/*$('#select-team').select('val', o.teamId);
						$('#select-teamUser').select('val', o.teamUserId);*/
						if (o._hurryFlag === 'on') {
							$('#hurryFlag1').attr('checked', true);
						} else {
							$('#hurryFlag1').removeAttr('checked');
						}
						if (o._timeoutFlag === 'on') {
							$('#timeoutFlag1').attr('checked', true);
						} else {
							$('#timeoutFlag1').removeAttr('checked');
						}
				...
				bindEvent : function() {
					...
					$('body').on('click', '.dorpdownbotms', function(event) {
				   	...
				   	.on('mouseleave', '.act-content-flowfrom', function(event) {
						...
						sessionStorage.removeItem('teamTaskListFormData');
						return true;
				   	}).on('click', '.showTaskPocess', function(event) {
				   		event.preventDefault();
				   		var pid = $(this).attr('data-pid');
                        var datas = pid.split(',');
				   		var w = $(window).width();
                        top.$.jBox.open("iframe:" + ctx + '/workflow/toTaskHistoicFlow?procInstId=' + datas[1] + '&taskId=' + datas[0] + '&from=teamtasklistNew', "任务进度", w, 600, {
							buttons:{"关闭":true},
							loaded: function(h){
